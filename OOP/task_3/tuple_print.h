...
auto print(std::basic_ostream<Ch,Tr>& out, const T& what) -> decltype(out << what){
  return out << what;
}

template<typename Ch, typename Tr, typename T, typename... Types>
auto print(std::basic_ostream<Ch,Tr>& out, const T& what, const Types& ... other) -> decltype(out << what) {
...
auto tuple_print(std::basic_ostream<Ch,Tr>& out, const std::tuple<Types...>& what, const seq<range...>) -> decltype(out){
  return print(out, std::get<range>(what)...);
}

template<typename Ch, typename Tr, typename... Types>
auto operator<<(std::basic_ostream<Ch,Tr>& out, const std::tuple<Types...>& what) -> decltype(out) {
